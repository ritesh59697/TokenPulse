    ...
    <tr onClick={onClick} onMouseEnter={onHover} onMouseLeave={onLeave}
      ...
      <td style={{ padding: "12px 20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: hovered ? "#60a5fa" : "white", transition: "color 0.15s" }}>{coin.name}</div>
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <span style={{ fontSize: 10, fontFamily: "'DM Mono', monospace", color: "#6b7280", textTransform: "uppercase" }}>{coin.symbol}</span>
      ...
      <td style={{ padding: "12px 20px", textAlign: "right", fontFamily: "'DM Mono', monospace", fontSize: 13, fontWeight: 500, color: "white" }}>{formatPrice(coin.current_price)}</td>
      ...
      <td style={{ padding: "12px 20px", textAlign: "right", fontFamily: "'DM Mono', monospace", fontSize: 13, color: "#9ca3af" }}>{formatLargeNumber(coin.total_volume)}</td>
      <td style={{ padding: "12px 20px", textAlign: "right", fontFamily: "'DM Mono', monospace", fontSize: 13, color: "#9ca3af" }}>{formatLargeNumber(coin.liquidity)}</td>
      <td style={{ padding: "12px 20px", textAlign: "right" }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 2 }}>
          <span style={{ fontSize: 12, fontFamily: "'DM Mono', monospace", color: "white" }}>{coin.txns24h?.toLocaleString()}</span>
...
export default function TrendingTokens({ coins, dexTokens, dexLoading, loading, onSelectCoin }) {
  ...
  return (
    <div className="card fade-in fade-in-3">
      <div style={{ padding: "20px 20px 0", display: "flex", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        ...
        {/* Tab switcher */}
        <div style={{ display: "flex", gap: 2, background: "#0b0f19", borderRadius: 10, padding: 3, border: "1px solid #1e2d45", marginLeft: "auto" }}>
          {TABS.map(t => (
        ...
        <span style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#4b5563" }}>
        ...
        {tab === "Market" ? (
              ...
              <tr style={{ borderBottom: "1px solid #1e2d45" }}>
                {COLS_MARKET.map((col, i) => (
                  <th key={col} onClick={() => toggleSort(SORT_KEYS[i])}
                    style={{ padding: "10px 20px", textAlign: i >= 2 ? "right" : "left", fontSize: 11, fontWeight: 600, color: "#4b5563", textTransform: "uppercase", letterSpacing: "0.07em", cursor: "pointer", whiteSpace: "nowrap", transition: "color 0.15s" }}
                ...
                : sorted.slice(0, 25).map((coin) => {
                    ...
                    return (
                      <tr key={coin.id} onClick={() => onSelectCoin?.(coin)}
                        ...
                        style={{ cursor: "pointer", background: isHov ? "rgba(59,130,246,0.04)" : "transparent", transition: "background 0.15s", borderBottom: "1px solid rgba(30,45,69,0.5)" }}>
                        <td style={{ padding: "13px 20px", fontFamily: "'DM Mono', monospace", fontSize: 12, color: "#6b7280" }}>{coin.market_cap_rank}</td>
                        <td style={{ padding: "13px 20px" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            <img src={coin.image} alt={coin.name}
                              style={{ width: 28, height: 28, borderRadius: "50%", border: isHov ? "1px solid rgba(59,130,246,0.4)" : "1px solid #1e2d45", transition: "border-color 0.2s" }}
                              onError={e => e.target.style.display = "none"} />
                            <div>
                              <div style={{ fontSize: 13, fontWeight: 600, color: isHov ? "#60a5fa" : "white", transition: "color 0.15s" }}>{coin.name}</div>
                              <div style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#6b7280", textTransform: "uppercase" }}>{coin.symbol}</div>
                            </div>
                          </div>
                        </td>
                        <td style={{ padding: "13px 20px", textAlign: "right", fontFamily: "'DM Mono', monospace", fontSize: 13, fontWeight: 500, color: "white" }}>{formatPrice(coin.current_price)}</td>
                        <td style={{ padding: "13px 20px", textAlign: "right" }}>
                          <span style={{ display: "inline-flex", alignItems: "center", gap: 3, padding: "3px 8px", borderRadius: 100, fontSize: 11, fontWeight: 600, background: pos ? "rgba(34,197,94,0.1)" : "rgba(239,68,68,0.1)", color: pos ? "#22c55e" : "#ef4444", border: `1px solid ${pos ? "rgba(34,197,94,0.2)" : "rgba(239,68,68,0.2)"}` }}>
                            {formatPercent(coin.price_change_percentage_24h)}
                          </span>
                        </td>
                        <td style={{ padding: "13px 20px", textAlign: "right", fontFamily: "'DM Mono', monospace", fontSize: 13, color: "#9ca3af" }}>{formatLargeNumber(coin.total_volume)}</td>
                        <td style={{ padding: "13px 20px", textAlign: "right", fontFamily: "'DM Mono', monospace", fontSize: 13, color: "#9ca3af" }}>{formatLargeNumber(coin.market_cap)}</td>
                      </tr>
                    );
                  })}
            </tbody>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 16 }}>
            <thead>
              <tr style={{ borderBottom: "1px solid #1e2d45" }}>
                {COLS_DEX.map((col, i) => (
                  <th key={col} style={{ padding: "10px 20px", textAlign: i === 0 ? "left" : "right", fontSize: 11, fontWeight: 600, color: "#4b5563", textTransform: "uppercase", letterSpacing: "0.07em" }}>{col}</th>
                ))}
              </tr>
            </thead>
            <tbody>
      ...
      {/* Click hint */}
      <div style={{ padding: "12px 20px", borderTop: "1px solid #1e2d45", textAlign: "center" }}>
