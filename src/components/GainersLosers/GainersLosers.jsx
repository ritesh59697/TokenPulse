...
function MiniList({ title, coins, positive, icon: Icon, iconColor, loading }) {
  return (
      ...
      <div className="space-y-2.5">
        {loading
          ...
          : coins.map((coin, idx) => (
              <div
                key={coin.id}
                className="flex items-center gap-3 p-2 rounded-xl hover:bg-card-hover transition-colors cursor-default group"
              >
                <div className="relative">
                  ...
                  />
                  <span className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-card text-[8px] font-bold text-muted flex items-center justify-center border border-card-border">
                    {idx + 1}
                  </span>
                </div>

                <div className="flex-1 min-w-0">
                  <div className="text-[13px] font-semibold text-white truncate leading-tight">{coin.name}</div>
...
export default function GainersLosers({ coins, loading }) {
